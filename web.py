...
data1=data.copy()
def volume_number():
    data1['ITEM_SALES_VOLUME'] = data1['ITEM_SALES_VOLUME'].apply(lambda x: x)
    ...
    data1['ITEM_SALES_VOLUME'] = data1['ITEM_SALES_VOLUME'].apply(tranAge)
    volume = data1.ITEM_SALES_VOLUME.value_counts()
...
def amount():
    ...
    data1['ITEM_SALES_AMOUNT'] = data1['ITEM_SALES_AMOUNT'].apply(tranAge)
    AMOUNT = data1.ITEM_SALES_AMOUNT.value_counts()
...
def cate():
    data1.CATE_NAME_LV1 = data1.CATE_NAME_LV1.replace({np.nan: 'missing'})
    cate = data1.CATE_NAME_LV1.value_counts()
...
def price_number():
    data1['ITEM_PRICE'] = data1['ITEM_PRICE'].apply(lambda x: x)
    ...
    data1['ITEM_PRICE'] = data1['ITEM_PRICE'].apply(tranAge)
    price = data1.ITEM_PRICE.value_counts()
...
def FAV_EVA():
    def tranAge(x):
        if 0 <= x < 100:
            ...
            x = 4
        return x
    data1['ITEM_FAV_NUM'] = data1['ITEM_FAV_NUM'].apply(tranAge)
    data1.ITEM_FAV_NUM = data1.ITEM_FAV_NUM.replace({np.nan: 'missing'})
    ITEM_FAV_NUM = data1.ITEM_FAV_NUM.value_counts()
    ...
    data1['TOTAL_EVAL_NUM'] = data1['TOTAL_EVAL_NUM'].apply(tranAge)
    data1.TOTAL_EVAL_NUM = data1.TOTAL_EVAL_NUM.replace({np.nan: 'missing'})
    TOTAL_EVAL_NUM = data1.TOTAL_EVAL_NUM.value_counts()
...
def Double_coordinates():
    ...
    st.markdown('#### 数据表展示')
    data1=data.head(100)
    data1.drop(columns=["Unnamed: 0","Unnamed: 0.1"])
    st.table(data1)
...
def Layouts_plotly():
    ...
    if add_selectbox == "缺失值":
        ...
        delivery()
    # 补充表单
    st.sidebar.button('异常数据表', on_click=Double_coordinates)
